...
class RevenueRefundAccountController extends Controller
{
    /**
     * Get Revenue Refund Account Report data
     */
    public function getData(Request $request)
    {
        try {
            ...
            // from monthly_fincances table for the selected year with dr_cr_code=5000 and dr_cr='DR'
            ...
            \Log::error('Error in RevenueRefundAccount getData: ' . $e->getMessage());
            return response()->json([
                'success' => false,
    ...
    public function getFilterOptions(Request $request)
    {
        try {
            ...
            \Log::error('Error in RevenueRefundAccount getFilterOptions: ' . $e->getMessage());
            return response()->json([
                'success' => false,
                'message' => $e->getMessage()
            ], 500);
    ...
    //         // Prepare CSV headers
    //         $headers = ['Revenue Code Name', 'Head-Program-Project-SubProject-Object'];
    //         for ($i = 1; $i <= 12; $i++) {
    ...
    //             ->header('Content-Disposition', "attachment; filename=revenue_refund_account_{$year}.csv");

    //     } catch (\Exception $e) {
    //         \Log::error('Error in RevenueRefundAccount exportCsv: ' . $e->getMessage());
    //         return response()->json([
    //             'success' => false,
    //             'message' => $e->getMessage()
    //         ], 500);
    ...
    public function exportCsv(Request $request)
    {
        try {
            ...
            $headers = ['Revenue Code Name', 'Revenue Code'];
            for ($i = 1; $i <= 12; $i++) {
