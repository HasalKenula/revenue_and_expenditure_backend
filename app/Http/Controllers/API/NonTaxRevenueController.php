...
class NonTaxRevenueController extends Controller
{
    /**
     * Get Non Tax Revenue Report data
     */
    public function getData(Request $request)
    {
        try {
            ...
            // Get all estimates with head between 2000 and 3000
            $estimates = Estimate::where('head', '>', 2000)
                ->where('head', '<', 3000)
                ->orderBy('head')
                ->orderBy('program')
                ->orderBy('project')
            ...
            \Log::error('Error in NonTaxRevenue getData: ' . $e->getMessage());
            return response()->json([
                'success' => false,
    ...
    public function getFilterOptions(Request $request)
    {
        try {
            ...
            \Log::error('Error in NonTaxRevenue getFilterOptions: ' . $e->getMessage());
            return response()->json([
                'success' => false,
                'message' => $e->getMessage()
            ], 500);
    ...
    //             ->header('Content-Disposition', "attachment; filename=non_tax_revenue_{$year}_{$monthName}.csv");

    //     } catch (\Exception $e) {
    //         \Log::error('Error in NonTaxRevenue exportCsv: ' . $e->getMessage());
    //         return response()->json([
    //             'success' => false,
    //             'message' => $e->getMessage()
    //         ], 500);
