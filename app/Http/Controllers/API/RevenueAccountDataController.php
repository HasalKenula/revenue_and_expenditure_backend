...
class RevenueAccountDataController extends Controller
{
    /**
    ...
    {
        try {
            Log::info('Fetching revenue account data', $request->all());
            
            $query = RevenueAccountData::with(['accountNumber', 'estimate']);
            ...
            $revenueData = $query->orderBy('id', 'desc')->paginate($perPage);

            Log::info('Revenue data fetched successfully', ['count' => $revenueData->count()]);

            return response()->json([
                'success' => true,
                'data' => $revenueData->items(),
                'pagination' => [
                    'current_page' => $revenueData->currentPage(),
                    'per_page' => $revenueData->perPage(),
                    'total' => $revenueData->total(),
                    'last_page' => $revenueData->lastPage(),
                ],
                'summary' => [
    ...
    {
        try {
            Log::info('Creating revenue account data', $request->all());
            ...
            $exists = RevenueAccountData::where('account_number_id', $request->account_number_id)
            ...
            $revenueData = RevenueAccountData::create([
            ...
            $revenueData->load(['accountNumber', 'estimate']);

            Log::info('Revenue data created successfully', ['id' => $revenueData->id]);
            ...
            Log::error('Error creating revenue data: ' . $e->getMessage());
            
            return response()->json([
                'success' => false,
                'message' => 'Error creating revenue data: ' . $e->getMessage()
            ], 500);
        }
    }
    ...
    {
        try {
            $revenueData = RevenueAccountData::with(['accountNumber', 'estimate'])->find($id);
    ...
    {
        try {
            $revenueData = RevenueAccountData::find($id);
            ...
            if ($request->filled('account_number_id') || $request->filled('estimate_id') || 
                ...
                $accountNumberId = $request->account_number_id ?? $revenueData->account_number_id;
                $estimateId = $request->estimate_id ?? $revenueData->estimate_id;
                $month = $request->month ?? $revenueData->month;
                $year = $request->year ?? $revenueData->year;

                $exists = RevenueAccountData::where('account_number_id', $accountNumberId)
            ...
            $revenueData->update($request->only([
                'account_number_id',
                'amount',
                'month',
                'year',
            ...
            Log::info('Revenue data updated successfully', ['id' => $revenueData->id]);
            ...
            Log::error('Error updating revenue data: ' . $e->getMessage());
            
            return response()->json([
                'success' => false,
                'message' => 'Error updating revenue data: ' . $e->getMessage()
            ], 500);
        }
    }
    ...
    public function destroy($id)
    {
        try {
            ...
            Log::info('Revenue data deleted successfully', ['id' => $id]);
            ...
            Log::error('Error deleting revenue data: ' . $e->getMessage());
            
            return response()->json([
                'success' => false,
                'message' => 'Error deleting revenue data: ' . $e->getMessage()
            ], 500);
        }
    }
    ...
    public function destroyMultiple(Request $request)
    {
        try {
            $validator = Validator::make($request->all(), [
                'ids' => 'required|array',
                'ids.*' => 'exists:revenue_account_data,id'
            ]);
            ...
            $deletedCount = RevenueAccountData::whereIn('id', $request->ids)->delete();

            Log::info('Multiple revenue data deleted', ['count' => $deletedCount]);
            ...
            Log::error('Error deleting multiple revenue data: ' . $e->getMessage());
    ...
    /**
     * Get revenue data by account number
     */
    public function getByAccountNumber($accountNumberId)
    {
        try {
            $revenueData = RevenueAccountData::with(['accountNumber', 'estimate'])
            ...
            return response()->json([
                ...
                'data' => $revenueData,
                'summary' => [
                    'total_amount' => $revenueData->sum('amount'),
                    'total_records' => $revenueData->count(),
                ]
            ]);
        } catch (\Exception $e) {
            Log::error('Error fetching revenue data by account: ' . $e->getMessage());
    ...
    public function getByEstimate($estimateId)
    {
        try {
            ...
            Log::error('Error fetching revenue data by estimate: ' . $e->getMessage());
    ...
    {
        try {
            Log::info('Fetching revenue summary');
            
            $summary = RevenueAccountData::with(['accountNumber', 'estimate'])
            ...
            $grandTotal = RevenueAccountData::sum('amount');
    ...
    {
        try {
            $months = RevenueAccountData::getAvailableMonths();
            $years = RevenueAccountData::getAvailableYears();
    ...
    public function export(Request $request)
    {
        try {
            ...
            Log::error('Error exporting revenue data: ' . $e->getMessage());
            
            return response()->json([
                'success' => false,
                'message' => 'Error exporting revenue data: ' . $e->getMessage()
            ], 500);
        }
    }
    ...
    {
        try {
            $totalRecords = RevenueAccountData::count();
            $totalAmount = RevenueAccountData::sum('amount');
            $averageAmount = $totalRecords > 0 ? $totalAmount / $totalRecords : 0;
            $maxAmount = RevenueAccountData::max('amount') ?? 0;
            $minAmount = RevenueAccountData::min('amount') ?? 0;
    ...
    {
        try {
            $records = RevenueAccountData::with(['accountNumber', 'estimate'])
                ->where('month', $month)
                ->orderBy('estimate_id', 'asc')
                ->get();
