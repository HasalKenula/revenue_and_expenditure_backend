...
class LocalGovTransferSummaryController extends Controller
{
    /**
     * Get Local Gov Transfer Summary Report data
     */
    public function getData(Request $request)
    {
        try {
            ...
            \Log::info('LocalGovTransferSummary getData called', [
                'year' => $year
            ]);
            ...
            return response()->json([
                'success' => true,
                'data' => [
                    'local_gov_transfer_summary' => $results,
                    'filters' => [
                        'year' => $year,
                    ]
                ]
            ...
            \Log::error('Error in LocalGovTransferSummary getData: ' . $e->getMessage());
            return response()->json([
                'success' => false,
    ...
    /**
     * Process data for Local Gov Transfer Summary
     */
    private function processData($year)
    {
        ...
        // Define the categories for Salary Reimbursement and Members Allowances
        $categories = [
        ...
        for ($month = 1; $month <= 12; $month++) {
            ...
            $resultRow = [
                ...
                'month_number' => $month,
                'total' => $monthlyTotal,
                'cumulative_expenditure' => $cumulativeTotal
            ];
    ...
    public function getFilterOptions(Request $request)
    {
        try {
            ...
            \Log::error('Error in LocalGovTransferSummary getFilterOptions: ' . $e->getMessage());
            return response()->json([
                'success' => false,
                'message' => $e->getMessage()
            ], 500);
        ...
        try {
            ...
            foreach ($results as $result) {
                $rowData = [
                    'Month' => $result['month'],
                    'Total' => $result['total'],
                    'Cumulative Expenditure' => $result['cumulative_expenditure']
                ];
