...
{
    /**
     * Get Other Department Debits data
     */
    public function getData(Request $request)
    {
        try {
            ...
            // where dr_cr_code = 1000, dr_cr = DR, and trno != head
            $records = MonthlyFincance::whereYear('created_at', $year)
                ->where('month', $month)
                ->where('dr_cr_code', 1000) // DR code
                ->where('dr_cr', 'DR')
                ...
                ->select(
                    ...
                    'project',
                    'object',
                    'sub_project as item', // Changed from 'item' to 'sub_project' as item
                    'funding as sub_object',
                    'cash_xe as debit_amount',
                    'subject',
                    'sn',
                    'dr_cr_code',
                    'dr_cr',
            ...
            \Log::error('Error in ODD getData: ' . $e->getMessage());
            return response()->json([
                'success' => false,
    ...
    {
        try {
            // Get available years from created_at where dr_cr_code = 1000 and dr_cr = DR
            $years = MonthlyFincance::where('dr_cr_code', 1000)
                ->where('dr_cr', 'DR')
            ...
            \Log::error('Error in ODD getFilterOptions: ' . $e->getMessage());
            return response()->json([
                'success' => false,
                'message' => $e->getMessage()
            ], 500);
